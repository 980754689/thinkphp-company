	...
	<h1>
		...
		<table class="table table-striped table-bordered table-hover">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					...
					<td><img src="/Public<?php echo ($vo["img"]); ?>" style='width: 40px;height:30px'></td>
					<td><?php echo ($vo["title"]); ?></td>
					<td><?php echo ($vo["user_name"]); ?></td>
					<td><?php if($vo["type_id"] == 0): ?>公司新闻
						<?php elseif($vo["type_id"] == 1): ?>行业动态
						<?php elseif($vo["type_id"] == 2): ?>常见问题<?php endif; ?>
					</td>
					<td><?php if($vo["is_valid"] == 1): ?>启用<?php else: ?>禁用<?php endif; ?></td>
					<td><?php if($vo["seat"] == 100): ?>100<?php else: ?>400<?php endif; ?></td>
					<td><?php echo (date("Y-m-d",$vo["created_time"])); ?></td>
					<td><?php echo (date("Y-m-d",$vo["updated_time"])); ?></td>
					<td>
						<a href="<?php echo U('New/show',array('id'=>$vo['id']));?>">查看</a>
						<a href="<?php echo U('New/edit',array('id'=>$vo['id']));?>">修改</a>
						<a href="<?php echo U('New/del',array('id'=>$vo['id']));?>">删除</a>
					</td>
