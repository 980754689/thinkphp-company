	...
	<h1>
		...
		<table class="table table-striped table-bordered table-hover">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					...
					<td><?php if($vo["address"] == 1): ?>品牌产品
						<?php elseif($vo["address"] == 2): ?>案例展示
						<?php elseif($vo["address"] == 3): ?>营销活动
						<?php elseif($vo["address"] == 4): ?>新闻动态
						<?php elseif($vo["address"] == 5): ?>关于我们<?php endif; ?></td>
					<td><?php echo (date("Y-m-d H:i",$vo["created_time"])); ?></td>
					<td><?php echo (date("Y-m-d H:i",$vo["updated_time"])); ?></td>
					<td>
						<a href="<?php echo U('Advert/del',array('id'=>$vo['id']));?>">删除</a>
						<a href="<?php echo U('Advert/edit',array('id'=>$vo['id']));?>">修改</a>
					</td>
