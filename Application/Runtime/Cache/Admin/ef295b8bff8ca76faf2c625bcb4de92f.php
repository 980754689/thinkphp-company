	...
	<h1>
		...
		<table class="table table-striped table-bordered table-hover">
			...
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
					...
					<td><?php if($vo["is_valid"] == 1): ?>启用
						<?php else: ?>禁用<?php endif; ?>
					</td>
					<td><?php echo (date("Y-m-d H:i",$vo["created_time"])); ?></td>
					<td><?php echo (date("Y-m-d H:i",$vo["updated_time"])); ?></td>
					<td>
						<a href="<?php echo U('Flow/show',array('id'=>$vo['id']));?>">查看</a>
						<a href="<?php echo U('Flow/edit',array('id'=>$vo['id']));?>">修改</a>
						<a href="<?php echo U('Flow/del',array('id'=>$vo['id']));?>">删除</a>
					</td>
